<template>
  <div>
      <el-row type="flex">
            <el-col :span="6">
                <div>
                    <el-input placeholder="请输入用例名" v-model="searchString">
                        <i class="el-icon-search el-input__icon" slot="suffix" @click="searchCase"></i>
                    </el-input>
                </div>
            </el-col>
      </el-row>
    ...
    >
        ...
        >  
            <template slot-scope="scope">
                <span v-if="scope.row.startingTime != null">{{formatDate(scope.row.startingTime)}}</span>
                <span v-if="scope.row.starting_time != null">{{formatDate(scope.row.starting_time)}}</span>
        ...
        > 
            <template slot-scope="scope">
                <el-button type="text" @click="sendTestCase(scope.row)">查看</el-button>
            </template>
        </el-table-column>
    </el-table>
      <br>
...
<script>
  export default {
    data() {
      return {
          tableData: [],
          searchFilters: ['性能测试', '接口测试', '功能测试', '流程测试'],
          searchString: '',
          currentPage: 0,
          totalPage: 1,
          softwareCodeFilters: [],
    ...
    created() {
        if(this.testCase !== null) {
            this.tableData = []
            this.tableData.push(this.testCase)
            this.sendTestCase(this.testCase)
        } else {
            this.getConfirmCase()
        }
    },
    ...
    methods: {
        ...
        getDateFilters(cases) {
            ...
            for(var i = 0; i < cases.length; i ++) {
                var time = '';
                if(cases[i].startingTime != undefined) {
                    time = cases[i].startingTime;
                    if(time !== null && time !== "") {
                    var dateFilter = {
                            text: time.split('T')[0],
        ...
        sendTestCase(id) {
            this.$emit('getTestCase', id);
        },
        getConfirmCase() {
            this.$axios.post('/case/getConfirmRecordCase',this.qs.stringify({activityId: this.$store.state.activityId, offset: this.currentPage, searchString: this.searchString, searchByString: this.searchFilters}, { indices: false }),
                {
                    header: {
                ...
                if (resp && resp.status == 200) {
                    ...
                    if(resp.data.status == 200){
                        this.tableData = resp.data.obj.content;
